module MainPageHelper
  ...
  def options_for_hot_water_display
    ...
    section_width = dimensions_reader("section-width").strip[0..-3].to_f
    section_padding = dimensions_reader("section-padding").strip[0..-3].to_f
            
    opts = {
      height: (full_heigth*remaining_hot_water/max_hot_water).ceil.to_s+"px" ,
      width: (section_width - 2 * section_padding).floor.to_s+"px" ,
